...
<div class="span8">
          <ul class="unstyled">
            <?php if(is_array($post_rs)): $i = 0; $__LIST__ = $post_rs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$post): $mod = ($i % 2 );++$i;?><li>
              <h2> <a href="__APP__/Index/show/post_id/<?php echo ($post["post_id"]); ?>"><?php echo ($post["title"]); ?></a></h2>
              <p class="text-left"><?php echo ($post["sn_title"]); ?></p>
              <p class="text-right"><a href="__APP__/Index/show/post_id/<?php echo ($post["post_id"]); ?>" class="label label-info" >点击查看</a>&nbsp;&nbsp;<em>浏览次数:</em><small class="badge badge badge-important"><?php echo ($post["view"]); ?></small>&nbsp;&nbsp;<em>评论:</em><small class="badge badge-success"><?php echo ($post["view"]); ?></small></p>
              <hr/>
